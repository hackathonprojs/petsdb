...
function ViewStockData(irisNative)
	{
        ...
        */
       var iter = irisNative.iterator('^pet',1).keys();
       var pet = iter.next();
       console.log(iter);
       while (!pet.done) {
        // console.log(pet.value[0]);
        let output = "Key="+pet.value
        let state = irisNative.isDefined('^pet',1,pet.value)
        if (state == 1 || state == 11) {
            output += " ; value="+irisNative.get('^pet',1,pet.value)
        }
